...
const PAGE_SHADOW_LEFT = 'shadow-[inset_-7px_0_15px_-7px_rgba(0,0,0,0.15),0_0_30px_rgba(0,0,0,0.3)]';
const PAGE_SHADOW_RIGHT = 'shadow-[inset_7px_0_15px_-7px_rgba(0,0,0,0.15),0_0_30px_rgba(0,0,0,0.3)]';
...
/* ── sub-component: one page face (left or right) ── */
function PageContent({
  ...
  if (!stage) {
    return (
      <div className={`w-full h-full ${PAGE_BG} flex items-center justify-center ${side === 'left' ? PAGE_SHADOW_LEFT : PAGE_SHADOW_RIGHT}`}>
  ...
  if (side === 'left') {
    return (
      <div className={`w-full h-full ${PAGE_BG} ${PAGE_SHADOW_LEFT} p-6 md:p-10 flex flex-col relative overflow-hidden`}>
        {/* page edge */}
  ...
  /* right page – image */
  return (
    <div className={`w-full h-full ${PAGE_BG} ${PAGE_SHADOW_RIGHT} p-6 md:p-10 flex flex-col items-center justify-center relative overflow-hidden`}>
...
export default function BookViewer() {
  ...
  const handleTransitionEnd = useCallback(() => {
    if (flipping === 'next') setCurrentPage((p) => p + 1);
    if (flipping === 'prev') setCurrentPage((p) => p - 1);
    setFlipping(null);
  }, [flipping]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-neutral-950 via-black to-neutral-950 flex flex-col items-center justify-center p-4 md:p-8 select-none">
      ...
      {totalPages === 0 ? (
        ...
        /* ── BOOK ── */
        <div
          className="w-full max-w-5xl flex-1 flex items-center justify-center"
          style={{ perspective: '2500px' }}
        >
          <div className="relative w-full" style={{ transformStyle: 'preserve-3d' }}>
            {/* Book body */}
            <div
              className="relative w-full flex min-h-[500px] md:min-h-[600px]"
              style={{ transformStyle: 'preserve-3d' }}
            >
              {/* ────── STATIC LEFT PAGE (always the "left" of current spread) ────── */}
              <div
                className="w-1/2 relative z-0 rounded-l-xl overflow-hidden"
                style={{ transformStyle: 'preserve-3d' }}
              >
                {/* When flipping prev, the static left shows the PREVIOUS spread's left */}
                <PageContent
                  side="left"
                  stage={flipping === 'prev' ? prevStage : currentStage}
                  playingAudio={playingAudio}
                  onPlayAudio={playAudio}
                />
              </div>

              {/* ────── STATIC RIGHT PAGE (always the "right" of current/next spread) ────── */}
              <div
                className="w-1/2 relative z-0 rounded-r-xl overflow-hidden"
                style={{ transformStyle: 'preserve-3d' }}
              >
                {/* When flipping next, the static right shows the NEXT spread's right */}
                <PageContent
                  side="right"
                  stage={flipping === 'next' ? nextStage : currentStage}
                  playingAudio={playingAudio}
                  onPlayAudio={playAudio}
                />
              </div>
              ...
              {flipping === 'next' && (
                <div
                  ...
                  style={{
                    ...
                    animation: 'pageFlipNext 1s cubic-bezier(0.645, 0.045, 0.355, 1) forwards',
                  }}
                ...
                >
                  ...
                  {/* Back face – next text (rotated 180 so it reads correctly when flipped) */}
                  <div
              ...
              {/* ═══════════════════════════════════════════
                  FLIPPING PAGE – PREV (left → right)
                  Front = current left (text)  [visible before flip]
                  Back  = prev right (image)   [visible after flip]
                  ═══════════════════════════════════════════ */}
              {flipping === 'prev' && (
                <div
                  ...
                  style={{
                    ...
                    transformStyle: 'preserve-3d',
                    animation: 'pageFlipPrev 1s cubic-bezier(0.645, 0.045, 0.355, 1) forwards',
                  }}
                  onAnimationEnd={handleTransitionEnd}
                >
          ...
          <div className="flex gap-1.5 max-w-xs overflow-x-auto py-2">
            {completedStages.map((_, idx) => (
              <button
                key={idx}
                onClick={() => {
                  if (idx === currentPage || flipping) return;
                  flipTo(idx, idx > currentPage ? 'next' : 'prev');
                }}
                className={`w-2.5 h-2.5 rounded-full transition-all ${
                  idx === currentPage ? 'bg-jade scale-125' : 'bg-neutral-700 hover:bg-neutral-500'
                }`}
              />
            ))}
          </div>
